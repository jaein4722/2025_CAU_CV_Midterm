...
from hyper_ultralytics.utils import (ASSETS, AUTOINSTALL, LINUX, LOGGER, ONLINE, ROOT, USER_CONFIG_DIR, SimpleNamespace,
...
def parse_requirements(file_path=ROOT.parent / 'requirements.txt', package=''):
    ...
    Args:
        ...
        package (str, optional): Python package to use instead of requirements.txt file, i.e. package='hyper_ultralytics'.
    ...
    Example:
        ```python
        from hyper_ultralytics.utils.checks import parse_requirements

        parse_requirements(package='hyper_ultralytics')
        ```
    """
...
def check_version(current: str = '0.0.0',
    ...
    if not current:  # if current is '' or None
        ...
        return True
    elif not current[0].isdigit():  # current is package name rather than version string, i.e. current='hyper_ultralytics'
        try:
...
def check_latest_pypi_version(package_name='hyper_ultralytics'):
    """
...
def check_pip_update_available():
    """
    Checks if a new version of the hyper_ultralytics package is available on PyPI.
    ...
    if ONLINE and is_pip_package():
        with contextlib.suppress(Exception):
            from hyper_ultralytics import __version__
            latest = check_latest_pypi_version()
            if check_version(__version__, f'<{latest}'):  # check if current version is < latest version
                LOGGER.info(f'New https://pypi.org/project/hyper_ultralytics/{latest} available 😃 '
                            f"Update with 'pip install -U hyper_ultralytics'")
                return True
    return False
...
def check_font(font='Arial.ttf'):
    ...
    url = f'https://hyper_ultralytics.com/assets/{name}'
...
def check_requirements(requirements=ROOT.parent / 'requirements.txt', exclude=(), install=True, cmds=''):
    ...
    Example:
        ...
        check_requirements('hyper_ultralytics>=8.0.0')

        # Check multiple packages
        check_requirements(['numpy', 'hyper_ultralytics>=8.0.0'])
        ```
    """
        ...
        if 'u.yaml' in file:
            ...
            if file != original_file and verbose:
                LOGGER.info(
                    ...
                    f'trained with https://github.com/hyper_ultralytics/hyper_ultralytics and feature improved performance vs '
                    f'standard YOLOv5 models trained with https://github.com/hyper_ultralytics/yolov5.\n')
    return file
...
def collect_system_info():
    ...
    from hyper_ultralytics.utils import ENVIRONMENT, is_git_dir
    from hyper_ultralytics.utils.torch_utils import get_cpu_info
    ...
    for r in parse_requirements(package='hyper_ultralytics'):
        try:
...
def check_amp(model):
    ...
    Example:
        ...
        from hyper_ultralytics.utils.checks import check_amp
