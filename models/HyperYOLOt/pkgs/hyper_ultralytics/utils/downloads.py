...
from hyper_ultralytics.utils import LOGGER, TQDM, checks, clean_url, emojis, is_online, url2file

# Define Ultralytics GitHub assets maintained at https://github.com/hyper_ultralytics/assets
GITHUB_ASSETS_REPO = 'hyper_ultralytics/assets'
...
def delete_dsstore(path, files_to_delete=('.DS_Store', '__MACOSX')):
    ...
    Example:
        ```python
        from hyper_ultralytics.utils.downloads import delete_dsstore
...
def zip_directory(directory, compress=True, exclude=('.DS_Store', '__MACOSX'), progress=True):
    ...
    Example:
        ```python
        from hyper_ultralytics.utils.downloads import zip_directory
...
def unzip_file(file, path=None, exclude=('.DS_Store', '__MACOSX'), exist_ok=False, progress=True):
    ...
    Example:
        ```python
        from hyper_ultralytics.utils.downloads import unzip_file
...
def check_disk_space(url='https://hyper_ultralytics.com/assets/coco128.zip', sf=1.5, hard=True):
    """
    Check if there is sufficient disk space to download and store a file.

    Args:
        url (str, optional): The URL to the file. Defaults to 'https://hyper_ultralytics.com/assets/coco128.zip'.
...
def get_google_drive_file_info(link):
    ...
    Example:
        ```python
        from hyper_ultralytics.utils.downloads import get_google_drive_file_info
...
def safe_download(url,
    ...
    Example:
        ```python
        from hyper_ultralytics.utils.downloads import safe_download

        link = "https://hyper_ultralytics.com/assets/bus.jpg"
...
def get_github_assets(repo='hyper_ultralytics/assets', version='latest', retry=False):
    """
    Retrieve the specified version's tag and assets from a GitHub repository. If the version is not specified, the
    function fetches the latest release assets.

    Args:
        repo (str, optional): The GitHub repository in the format 'owner/repo'. Defaults to 'hyper_ultralytics/assets'.
    ...
    Example:
        ```python
        tag, assets = get_github_assets(repo='hyper_ultralytics/assets', version='latest')
        ```
    """
...
def attempt_download_asset(file, repo='hyper_ultralytics/assets', release='v0.0.0', **kwargs):
    """
    ...
    Example:
        ```python
        file_path = attempt_download_asset('yolov5s.pt', repo='hyper_ultralytics/assets', release='latest')
        ```
    """
    from hyper_ultralytics.utils import SETTINGS  # scoped for circular import
...
def download(url, dir=Path.cwd(), unzip=True, delete=False, curl=False, threads=1, retry=3):
    ...
    Example:
        ```python
        download('https://hyper_ultralytics.com/assets/example.zip', dir='path/to/dir', unzip=True)
        ```
    """
    dir = Path(dir)
