...
from .pkgs.train import run as lightweightob_train
from .pkgs.models.experimental import attempt_load
...
def train_model(ex_dict, config: ModelConfig):
    ...
    lightweightob_config = config.convert_to_lightweightob_format(ex_dict, name)

    # YOGA train.py의 run 인터페이스 호출
    sys.argv = [''] # reset jupyter's system argv
    lightweightob_train(**lightweightob_config)
